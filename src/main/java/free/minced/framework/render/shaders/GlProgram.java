...
import free.minced.mixin.accesors.ShaderProgramAccessor;
...
public class GlProgram {
    ...
    public GlProgram(Identifier id, VertexFormat vertexFormat) {
        REGISTERED_PROGRAMS.add(new Pair<>(resourceFactory -> {
            try {
                return new ExtShaderProgram(resourceFactory, id.toString(), vertexFormat);
            } catch (IOException e) {
    ...
    public static class ExtShaderProgram extends ShaderProgram {
        private ExtShaderProgram(ResourceFactory factory, String name, VertexFormat format) throws IOException {
