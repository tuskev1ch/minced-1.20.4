...
public abstract class MixinWorldRenderer {
    ...
    private static int getLightmapCoordinatesModifySkyLight(int sky) {
        if (Minced.getInstance().getModuleHandler().get(FullBright.class).isEnabled())
            return (Minced.getInstance().getModuleHandler().get(FullBright.class).brightness.getValue().intValue());
        return sky;
    }

    @ModifyArg(method = "render", at = @At(value = "INVOKE", target = "Lnet/minecraft/client/render/WorldRenderer;setupTerrain(Lnet/minecraft/client/render/Camera;Lnet/minecraft/client/render/Frustum;ZZ)V"), index = 3)
    private boolean renderSetupTerrainModifyArg(boolean spectator) {
        return Minced.getInstance().getModuleHandler().get(FreeCam.class).isEnabled() || spectator;
    }
